      ...
      <div className="relative z-10">
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
          ...
          <div className="flex-1 text-center sm:text-left">
            <h2 className="text-xl sm:text-2xl font-bold">{user?.name || 'Finda'}</h2>
        ...
        <CardContent className="p-4 sm:p-5 flex flex-col items-center text-center">
          ...
          <div
            className={`relative flex h-16 w-16 sm:h-20 sm:w-20 items-center justify-center rounded-full mb-3 ${
              badge.earned
                ? `bg-gradient-to-br ${categoryIconColors[badge.category]} shadow-lg`
                : 'bg-muted'
            }`}
          >
        ...
        <div className="flex flex-col items-center text-center py-4">
          ...
          <motion.div
            ...
            className={`relative flex h-24 w-24 items-center justify-center rounded-full mb-4 ${
              badge.earned
                ? `bg-gradient-to-br ${categoryIconColors[badge.category]} shadow-xl`
                : 'bg-muted'
            }`}
          >
...
export function GamificationSystem({ onBack }: GamificationSystemProps) {
  ...
  useEffect(() => {
    fetch('/api/gamification')
      ...
      .then((data) => {
        if (data) {
          setRealData(data.profile)
          setRealLeaderboard(data.leaderboard || [])
        }
      })
      .catch((e) => console.error('Erreur chargement gamification:', e))
      .finally(() => setLoadingGamification(false))
  }, [])

  const realBadges: BadgeType[] = realData?.badges || []

  const profile: GamificationProfile = {
    level: realData?.level || { level: 1, title: 'Debutant', xp: 0, xpToNext: 200 },
    points: realData?.points || 0,
    badges: realBadges,
    streak: 0,
    streakMax: 0,
    rank: realData?.rank || 0,
  }
  const filteredBadges = useMemo(() => {
    if (badgeFilter === 'all') return realBadges
    return realBadges.filter((b) => b.category === badgeFilter)
  }, [badgeFilter, realBadges])
  const earnedCount = realBadges.filter((b) => b.earned).length
