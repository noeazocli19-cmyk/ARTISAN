...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!title || !description || !category) {
      return NextResponse.json({ error: 'Titre, description et cat̩gorie sont requis' }, { status: 400 });
    }
    ...
    return NextResponse.json({ error: 'Erreur lors de la cr̩ation de la mission' }, { status: 500 });
  }
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: 'Erreur lors de la r̩cup̩ration des missions' }, { status: 500 });
  }
}
